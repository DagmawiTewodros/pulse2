...
export function AlternatingFeatures() {
  return (
    <section className="py-12 sm:py-16 md:py-32">
      <div className="container-page px-4 sm:px-6 space-y-12 sm:space-y-16 md:space-y-32">
        {services.map((s, idx) => {
          ...
          return (
            <div
              key={s.id}
              id={s.id}
              className="grid gap-8 sm:gap-12 md:grid-cols-2 md:gap-16 items-center scroll-mt-24"
            >
              <Reveal className={reversed ? "md:order-2" : ""}>
                <span className="eyebrow">{s.eyebrow}</span>
                <h2 className="mt-3 text-xl sm:text-2xl md:text-5xl font-semibold tracking-tight">
                  {s.title}
                </h2>
                ...
                <ul className="mt-6 sm:mt-8 space-y-2 sm:space-y-3">
                  {s.bullets.map((b) => (
                    <li key={b} className="flex items-start gap-3">
                      ...
                      />
                      <span className="text-sm sm:text-base text-[color:var(--ink)]">{b}</span>
                    </li>
                  ))}
                </ul>
              </Reveal>
...
function BrowserMockup() {
  return (
    <div className="rounded-2xl border border-border bg-white shadow-[0_30px_80px_-40px_rgba(20,20,60,0.3)] overflow-hidden scale-90 sm:scale-95 md:scale-100 origin-top">
      <div className="flex items-center gap-2 border-b border-border px-3 sm:px-4 py-2 sm:py-3">
...
function PhoneMockup() {
  return (
    <div className="flex justify-center">
      <div className="w-[200px] sm:w-[240px] md:w-[280px] rounded-[2rem] sm:rounded-[2.5rem] border-4 sm:border-6 md:border-8 border-[color:var(--ink)] bg-white shadow-[0_30px_80px_-40px_rgba(20,20,60,0.4)] overflow-hidden">
...
function ArticleMockup() {
  return (
    <div className="rounded-2xl border border-border bg-white shadow-[0_30px_80px_-40px_rgba(20,20,60,0.3)] p-4 sm:p-6 md:p-8">
